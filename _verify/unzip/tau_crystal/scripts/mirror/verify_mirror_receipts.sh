...
[ -s "$meta" ] || { echo "[err] no mirror meta found; run mirror_receipts.sh"; exit 2; }
remote=$(sed -n "s/^remote: //p" "$meta" | head -n 1)
want_sha=$(sed -n "/^## mirror (v1)$/,/^## / s/^sha256: //p" "$man" | head -n 1)
have_sha=$(scripts/sha256.sh "$meta")
[ "$have_sha" = "$want_sha" ] || { echo "[FAIL] mirror meta hash mismatch; want: $want_sha, have: $have_sha"; exit 1; }
while IFS= read -r line; do
  ...
  have=$(scripts/sha256.sh "$tmp")
  [ "$have" = "$sha" ] || { echo "[FAIL] receipt $file hash mismatch; want: $sha, have: $have"; rm -f "$tmp"; exit 1; }
  rm -f "$tmp"
