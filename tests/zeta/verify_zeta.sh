...
# Compute -ζ'(0) from eigenvalues and ln det(A) from det file.
sumln=$(awk 'NR>1{acc+=log($1)} END{printf "%.12f",(acc+0)}' "$eigs")
neg_zeta_prime=$(awk -v s="$sumln" 'BEGIN{printf "%.12f", s}')   # -ζ'(0) = Σ ln λ
...
# Emit curvature TSV: mean-centered log-eigs (sum=0), so it integrates with the budget.
