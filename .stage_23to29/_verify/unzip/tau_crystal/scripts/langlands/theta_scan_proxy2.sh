...
sec="analysis/reciprocity_second.env"

readv(){ f="$1"; k="$2"; sed -n "s/^${k}=\\(.*\\)$/\\1/p" "$f" 2>/dev/null | head -n1; }

S1="$(readv "$best" BEST_S_MICRO)"
T1="$(readv "$best" BEST_T_MICRO)"
S2="$(readv "$sec"  BEST_S_MICRO)"
T2="$(readv "$sec"  BEST_T_MICRO)"
...
S_MIN=$(( $(min "$S1" "$S2") - padS ))
S_MAX=$(( $(max "$S1" "$S2") + padS ))
T_MIN=$(( $(min "$T1" "$T2") - padT ))
T_MAX=$(( $(max "$T1" "$T2") + padT ))

mkdir -p analysis
: > "$TMP"
printf "%s\t%s\t%s\t%s\t%s\t%s\t%s\n" "S_micro" "T_micro" "diff" "mA" "mB" "nA" "nB" >> "$TMP"

# GNU seq syntax: START STEP END
for S in $(seq "$S_MIN" "$stepS" "$S_MAX"); do
  for T in $(seq "$T_MIN" "$stepT" "$T_MAX"); do
    ds1=$(( S - S1 )); ds1=${ds1#-}
    dt1=$(( T - T1 )); dt1=${dt1#-}
