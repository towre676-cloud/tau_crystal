...
WF="${2:-.github/workflows/receipt.yml}"
RID="$(gh run list --workflow "$WF" --branch "$BRANCH" -L 1 --json databaseId --template '{{- if gt (len .) 0 -}}{{printf "%.0f" (index . 0).databaseId}}{{end -}}' 2>/dev/null || true)"
...
[ -s "$REC" ] || { echo "[err] no receipt.json in artifact"; exit 3; }
