...
RID="$(gh run list --workflow "$WF" --branch "$BRANCH" -L 1 \
  --json databaseId \
  --template '{{- if gt (len .) 0 -}}{{printf "%.0f" (index . 0).databaseId}}{{end -}}' \
  2>/dev/null || true)"
...
if ! gh run download "$RID" --name smartcache-receipt -D .artifact_tmp; then
  ...
  gh run view "$RID" --log-failed || gh run view "$RID" --log || true
  exit 2
fi
REC="$(find .artifact_tmp -type f -name 'receipt.json' | head -n1 || true)"
[ -s "${REC:-}" ] || { echo "[err] receipt.json missing in artifact"; exit 3; }
