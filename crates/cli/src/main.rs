...
fn ledger_seal(args: SealArgs) {
    ...
    let mut leaves = vec![];
    for path in args.capsules.iter() {
        let name = path.to_string_lossy().into_owned();
        let val: serde_json::Value = serde_json::from_slice(&std::fs::read(path).unwrap()).unwrap();
        let bytes = tau_core::jcs::to_vec(&val);
        let hash = tau_core::sha2_256(&bytes);
        let sha256_hex = hex::encode(hash);
        leaves.push(Leaf { name, sha256_hex });
    }

    leaves.sort_by(|a, b| a.name.cmp(&b.name));
    let master_hash = tau_core::sha2_256(&tau_core::jcs::to_vec(&leaves));
    let master = Master {
        schema_version: "0.1.0",
        kind: "MASTER",
        leaves,
        master_hash: hex::encode(master_hash),
    };

    let out = tau_core::jcs::to_vec(&master);
    println!("{}", String::from_utf8(out.clone()).unwrap());
    eprintln!("{}", hex::encode(tau_core::sha2_256(&out)));
}
