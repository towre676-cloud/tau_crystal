...
symmetry(){ L="$1"; R="$2"; EPS="$3"; [ -s "$L" ] && [ -s "$R" ] && [ -n "$EPS" ] || { echo "err"; exit 2; }
...
stability(){ T1="$1"; T2="$2"; DEL="$3"; [ -s "$T1" ] && [ -s "$T2" ] && [ -n "$DEL" ] || { echo "err"; exit 2; }
...
similar(){ X="$1"; Y="$2"; TAU="$3"; [ -s "$X" ] && [ -s "$Y" ] && [ -n "$TAU" ] || { echo "err"; exit 2; }
