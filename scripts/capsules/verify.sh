...
tail -n +2 "$IDX" | tr -d "\r" | awk 'BEGIN{OFS="\t"} {sub(/[ \t]+$/,"",$0); if(NF>0) print $0}' | LC_ALL=C sort -u >> "$TMP/.norm.tsv"
CANON=$(sha256sum "$TMP/.norm.tsv" | awk "{print \$1}")
STORED=$(tr -d "\r" < "$SIG" | awk "{print \$1}")
if [ "$CANON" = "$STORED" ]; then echo "[CAPVERIFY] ok"; exit 0; fi
echo "[CAPVERIFY] mismatch"; echo "canon=$CANON"; echo "store=$STORED"
