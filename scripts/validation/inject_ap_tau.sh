...
tail -n +2 "$CSV" | while IFS=, read -r -a F; do [ "${#F[@]}" -ge 1 ] || continue; face="${F[0]}"; [ -n "$face" ] || continue; toks=""; for ((i=1;i<${#H[@]};i++)); do p="${H[$i]}"; v="${F[$i]:-}"; [ -n "$v" ] && toks="$toks ap_tau_p${p}:${v}"; done; printf "%s\t%s\n" "$face" "${toks# }" >> "$MAP"; done
