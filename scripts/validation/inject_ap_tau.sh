...
TMP="$(mktemp)"; MAP="$(mktemp)"
header="$(head -n1 "$CSV")"; IFS=, read -r -a H <<<"$header"; [ "${#H[@]}" -ge 2 ] || { echo "bad header"; exit 1; }
tail -n +2 "$CSV" | while IFS=, read -r -a F; do [ "${#F[@]}" -ge 1 ] || continue; face="${F[0]}"; [ -n "$face" ] || continue; tokens="";
  for ((i=1;i<${#H[@]};i++)); do p="${H[$i]}"; v="${F[$i]:-}"; if [ -n "$v" ]; then tokens="$tokens ap_tau_p${p}:${v}"; fi; done
  printf "%s\t%s\n" "$face" "${tokens# }" >> "$MAP"
done
awk -F"\t" -vOFS="\t" -v MAP="$MAP" 'BEGIN{while((getline<MAP)>0){m[$1]=$2}}{k=$1; if(k in m && m[k]!=""){ $0=$0"\t"m[k] } print }' "$REC" > "$TMP"
mv "$TMP" "$REC"; rm -f "$MAP"
