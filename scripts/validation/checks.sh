...
for D in "$ROOT"/face*/; do [ -d "$D" ] || continue; face="$(basename "$D")"; AP="$D/a_p.tsv"; [ -s "$AP" ] || { echo "WARN $face: missing a_p.tsv"; continue; }
  while IFS=$'\t' read -r p ap; do
    case "$p" in ""|[!0-9]*) continue;; esac
    intok=0; case "$ap" in -|"" ) intok=0;; -*|[0-9]*) if printf "%s" "$ap" | grep -Eq "^-?[0-9]+$"; then intok=1; fi;; esac
    m691="skip"; if [ "$p" -ge 2 ]; then
    ...
    mult="skip"; if [ -s "$D/a_n.tsv" ]; then
      # optional Hecke check if a_{pq} present
      true
    fi
    stab="skip"; if [ -s "$AP.prev" ]; then
      if grep -qE "^${p}[[:space:]]+${ap}$" "$AP.prev"; then stab=ok; else stab=FAIL; fi
    fi
