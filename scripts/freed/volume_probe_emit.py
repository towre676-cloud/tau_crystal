...
def main():
    ...
    with open(pre.with_suffix(".json.tmp"),"w",encoding="utf-8") as f:
        json.dump({"volume":Vpre,"note":"volume_probe neutral (base from |logB| or 0)"},f,indent=2)
    os.replace(pre.with_suffix(".json.tmp"), pre)
    with open(post.with_suffix(".json.tmp"),"w",encoding="utf-8") as f:
        json.dump({"volume":Vpost,"note":"volume_probe neutral (base from |logB| or 0)"},f,indent=2)
