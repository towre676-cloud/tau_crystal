...
def main():
    ...
    run_id=f"relfun_{ts}"
    # conservative defaults; env overridable
    ...
    out={"run_id":run_id,"inputs":{"mu0":mu0,"b":b,"ell1":ell1,"ell2":ell2},
         "mu_total":mu_12,"mu_compose":mu_1_2,"residual":resid,
         "pass":resid<=1e-12}
    outp=f"analysis/freed/{run_id}.json"
    with open(outp,"w",encoding="utf-8") as f: json.dump(out,f,indent=2)
    mani={"run_id":run_id,"timestamp_utc":ts,"claims":{"relative_functor":"mu(ell1+ell2)=mu(ell2;mu(ell1))"},
