...
DL=".tau_ledger/delta.tsv"; SL=".tau_ledger/src_leaf.tsv"; DLf=".tau_ledger/dst_leaf.tsv"; MP=".tau_ledger/morphism_map.tsv"; TC=".tau_ledger/tau_cert.tsv"; OUT=".tau_ledger/delta_report.md"
...
: > "$OUT"
printf "# Cohomology Δ report (v2)\n\n" >> "$OUT"
printf "- ‖Δ‖₁: %s\n- support: %s (pos=%s, neg=%s)\n- leaves: src=%s, dst=%s, mapped=%s\n" "$L1" "$SUP" "$POS" "$NEG" "$SRCN" "$DSTN" "$MAT" >> "$OUT"
if [ -n "$TAUS$TAUD$TDR$LBD" ]; then printf "- τ_src: %s\n- τ_dst: %s\n- |Δτ|: %s\n- λ: %s\n" "$TAUS" "$TAUD" "$TDR" "$LBD" >> "$OUT"; fi
