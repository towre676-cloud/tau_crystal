...
euler_L_at_s() { # s ∈ {0.5±δ, 1}, panel tsv path, cutoff rows
  awk -v s="$1" -v CUT="$3" '
    BEGIN{FS="\t"; used=0; sumlog=0}
    NR==1{next}
    {
      if(used>=CUT) next
      p=$1+0; ap=$2+0
      den = (s==0.5 ? (2.0 - ap/sqrt(p)) : (1.0 - ap/p + 1.0/p))
      if(den<=0) den=1e300
      sumlog += -log(den); used++
    }
...
for leaf in analysis/atlas/*/leaf.json; do
  ...
  bound=$(jq -r '.numerics.tail_proxy.half' "$leaf")
  rows=$(awk 'END{print NR-1}' "$panel"); [ "$rows" -gt 0 ] || continue
  cut=$rows

  L0=$(euler_L_at_s 0.5 "$panel" "$cut")
  Lp=$(euler_L_at_s "$(awk -v d="$delta" 'BEGIN{print 0.5+d}')" "$panel" "$cut")
  Lm=$(euler_L_at_s "$(awk -v d="$delta" 'BEGIN{print 0.5-d}')" "$panel" "$cut")
  slope=$(awk -v lp="$Lp" -v lm="$Lm" -v d="$delta" 'BEGIN{printf "%.16f", (lp-lm)/(2*d)}')

  # Candidate: even parity (when known), |L(1/2)| < bound, |slope| < bound/delta
