...
bestd=999999999; best=""
exec 9<"$SRC"; IFS= read -r _ <&9 || true
while IFS= read -r line <&9; do
  [ -z "${line:-}" ] && continue
  first="${line%%[ 	]*}"; case "$first" in S_micro*|"#"*) continue;; esac
  set -- $line; S="${1:-}"; T="${2:-}"; D="${3:-}"; mA="${4:-}"; mB="${5:-}"; nA="${6:-}"; nB="${7:-}"
  [ -z "$S" ] || [ -z "$D" ] && continue
  D=$(( D + 0 ))
  if [ "$D" -lt "$bestd" ]; then bestd="$D"; best="$line"; fi
done; exec 9<&-
[ -n "$best" ] || { echo "[err] no data rows in TSV" >&2; exit 3; }
mkdir -p analysis
: > analysis/reciprocity_best.tsv; printf "%s\n" "S_micro\tT_micro\tdiff\tmA\tmB\tnA\tnB" >> analysis/reciprocity_best.tsv
printf "%s\n" "$best" >> analysis/reciprocity_best.tsv
set -- $best; S="$1"; T="$2"; D="$3"; mA="$4"; mB="$5"; nA="$6"; nB="$7"
micro_to_dec(){ local x="$1" s=""; [ "$x" -lt 0 ] && s="-" && x=$(( -x )); local i=$(( x/1000000 )); local f=$(( x%1000000 )); printf "%s%d.%06d" "$s" "$i" "$f"; }
