...
awk 'NR==1{next} {L[NR]=$0; n=NR} END{for(i=2;i<=n;i++) for(j=i+1;j<=n;j++){split(L[i],a,"\t"); split(L[j],b,"\t"); ds=a[2]-b[2]; dt=a[3]-b[3]; if(ds<0) ds=-ds; if(dt<0) dt=-dt; if(ds+dt<=20000) print "  \"" a[1] "\" -- \"" b[1] "\";"; }}' "$out" >> "$dot"
printf "%s\n" "}" >> "$dot"; echo "[defring] wrote $out and $dot"
