...
# ---------- gates & budgets ----------
: "${CI_AUTOMORPHIC:=1}"           # set to 0 to disable
: "${S_PAD:=50000}" : "${T_PAD:=50000}"
: "${S_STEP:=2000}" : "${T_STEP:=2000}"
: "${CI_AUTOMORPHIC_BUDGET:=45s}"  # if 'timeout' exists
...
# ---------- metrics ----------
rows=0; [ -f "$scan" ] && rows="$(wc -l < "$scan" 2>/dev/null || echo 0)"
data_rows=$(( rows>0 ? rows-1 : 0 ))

morse="analysis/morse_crit.tsv"
minima_count=0
[ -f "$morse" ] && minima_count="$(grep -Ec $'\tmin(imum)?\b' "$morse" 2>/dev/null || echo 0)"

best_delta=""; best_S="$S1"; best_T="$T1"
if [ -f "$morse" ] && [ "$minima_count" -gt 0 ]; then
  tail -n +2 "$morse" | while IFS=$'\t' read -r S T typ Delta _rest; do
    case "$typ" in *min* )
      if [ -z "$best_delta" ] || [ "$Delta" -lt "$best_delta" ]; then
        best_delta="$Delta"; best_S="$S"; best_T="$T"
      fi
    esac
    echo "__B $best_delta $best_S $best_T"
  done | tail -n1 | { read _ tag_delta tag_S tag_T; best_delta="$tag_delta"; best_S="$tag_S"; best_T="$tag_T"; }
fi
: "${best_delta:=0}"
...
# optional ledger anchor if present
