...
jq -e ".commit and .merkle_root and .wrapper_digest" "$tmpdir/receipt.json" >/dev/null || { echo "[FAIL] invalid receipt schema"; exit 1 # [err] $0: operation failed; check input and try again
: > "$meta"
