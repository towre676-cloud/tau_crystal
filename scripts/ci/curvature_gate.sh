...
bad="$(awk -F\"\t\" 'function finite(x){return (x==x) && (x>-1e308) && (x<1e308)} NR>1{c++; for(i=1;i<=NF;i++){x=$i+0; s[i]+=x; ss[i]+=x*x}} END{ if(c<1){print 1; exit} for(i=1;i<=NF;i++){ m=s[i]/c; v=(ss[i]/c - m*m); if(v<0) v=0; if(!finite(m) || !finite(v)) {print 1; exit} } print 0 }' "$tmp")"
