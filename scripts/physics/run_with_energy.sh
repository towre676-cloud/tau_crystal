...
PRE="$pre" POST="$post" PYTHONIOENCODING=UTF-8 python3 - <<'PY'
import json, os, math, pathlib
p = pathlib.Path(".tau_ledger/physics/post_meas.json")
doc = {"measured":{}}
try:
...
pre=float(os.environ.get("PRE","-1")); post=float(os.environ.get("POST","-1"))
