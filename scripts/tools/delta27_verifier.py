...
def meq(X,Y): return X[0][0]==Y[0][0] and X[0][1]==Y[0][1] and X[1][0]==Y[1][0] and X[1][1]==Y[1][1]
def mpow(X,k):
  R=[[1%mod,0],[0,1%mod]]
  for _ in range(int(k)):
    R=m2mul(R,X)
  return R
# robust matrix order finder; GL(2,3) has order 48
def morder(X,limit=48):
  I=[[1%mod,0],[0,1%mod]]
  for k in range(1,limit+1):
    if meq(mpow(X,k), I): return k
  return 0
I=[[1%mod,0],[0,1%mod]]
A1=[row[:] for row in A]; A2=m2mul(A1,A1)
Ao=morder(A1,48)
detA=det2(A1)
...
chk={
 "apow3_is_I": {"obs": True if meq(mpow(A1,3), [[1%mod,0],[0,1%mod]]) else False,
                 "exp": True,
                 "ok": True if meq(mpow(A1,3), [[1%mod,0],[0,1%mod]]) else False},
 "A_order_divides_48": {"obs": (48 % Ao == 0), "exp": True, "ok": (48 % Ao == 0)},
